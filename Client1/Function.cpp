...
void booking(CSocket& connector)
{
	// Choose hotel
	char* username;
	string_to_char(clientA.username, username);
...
// Gửi đi số phòng cần đặt
	...
	for (int i = 0; i < number_of_room_booking; i++)
	{
		...
		do
		{
			...
			do
			{
				...
				{
					cout << "Error date! Please try again!" << endl;
				}
		...
		connector.Send((char*)note, strlen(note), 0);
		delete[] note;
	}
	...
	//getchar();
	// Hoa don
	cout << "-Invoice of client!" << endl;;
	double total_money = 0;
	connector.Receive((char*)&total_money, sizeof(double), 0);
	cout << fixed << setprecision(2) << "Total: " << total_money << " USD" << endl;
	cout << "Have good vacation!" << endl;
	return;
}
